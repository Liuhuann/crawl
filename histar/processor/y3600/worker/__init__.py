...
class ReboWork(object):
    ...
    def process_content(self):
        ...
        content = content.replace("'",'"')
        index = content.find('div class="fm_b2 p20"')
        if index:
            content = content[index:]

        one_piece_pattern = '<DIV class=.*?><UL.*?</DIV>'
        url_pattern ='<OL><a href="(.*?)"'
        title_pattern = '<OL><a href.*?>(.*)</a></OL>'
